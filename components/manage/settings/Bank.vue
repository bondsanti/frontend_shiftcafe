...
<div class="ma-3" style="height: 100%">
    ...
    <v-overlay :absolute="absolute" :opacity="opacity" :value="overlay" dark>
        <v-dialog v-model="dialogView" color="primary" max-width="600px" persistent>
            <v-card style="backdrop-filter:blur(5px); background-color:rgba(255,255,255,0.9);">
                <v-form>
                    <v-card-title>
                        <h3 class="ma-2 ">
                    ...
                    <v-card-text>
                        ...
                        <v-row>
                            ...
                            <v-col cols="6" class="flex-grow-0 flex-shrink-0 text-center mt-6">
                                <v-img :src="
                      `${$nuxt.context.env.config.IMG_URL}${itemBy.img_cover}`
                    " class="mt-1 mb-1 rounded-xl" aspect-ratio="1" width="200px" height="200px" contain />
                            </v-col>
                        </v-row>
                    </v-card-text>
                    <v-card-actions>
                        <v-btn class="ma-1" style="  background-color: #00b894; color: #fff;border-radius: 0.25rem; padding: 0.5rem 1rem; border: none; outline: none;" dark @click="
                  closeviev();
                  overlay = false;
                ">
    ...
    <v-dialog v-model="dialog" max-width="700px" persistent>
        <v-card style="backdrop-filter:blur(5px); background-color:rgba(255,255,255,0.9); border-radius: 0.25rem; ">
            <v-card-title>
                <h2 class="text-h5 ">
            ...
            <v-card-text>
                <div>
                    <v-row>
                        <v-col cols="12" sm="6">
                            <v-text-field outlined label="ชื่อบัญชี" v-model="bankitem.bank_name" required  append-icon="mdi-credit-card"></v-text-field>
                        </v-col>

                        <v-col cols="12" sm="6">
                            <v-text-field outlined label="เลขบัญชี"  v-model="bankitem.bank_number" required  append-icon="mdi-credit-card-plus-outline"></v-text-field>
                        </v-col>

                        <v-row>
                            <v-col cols="12" sm="6" md="6">
                                <h3 class="text-center  ml-12 mb-6">
                                    <v-icon class="ma-2" >mdi-credit-card-scan-outline</v-icon>รูปคิวอาร์โค้ด
                                </h3>
                                <v-img v-if="imageURL" :src="imageURL" contain max-height="300px" max-width="250px" class="mb-3">
                                </v-img>
                            ...
                            <v-col cols="12" sm="6">
                                ...
                                <v-img v-if="imageURL2" :src="imageURL2" contain max-height="300px" max-width="250px" class="mb-3"></v-img>
                                <input accept="image/*" type="file" @change="onFileSelected2" />
                            </v-col>
                        </v-row>
                    </v-row>
                </div>
            ...
            <v-card-actions>
                <v-btn class="ma-1" color="primary" dark @click="
              close();
              overlay = false;
            " style=" padding: 0.5rem 1rem; border: none; outline: none;">
                ...
                <v-btn class="ma-1" color="info2" @click="
              save();
              overlay = false;
    ...
    <v-dialog v-model="dialogDelete" max-width="350px" persistent>
        <v-card style="backdrop-filter:blur(5px); background-color:rgba(255,255,255,0.9); border-radius: 0.25rem; ">
            ...
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="info2" class="ma-2" @click="
              closeDelete();
              overlay = false;
            " style="color: #fff;border-radius: 0.25rem; padding: 0.5rem 1rem; border: none; outline: none;">
                    <v-icon aria-hidden="false" class="mx-2"> mdi-close-box </v-icon>ยกเลิก
                </v-btn>
                <v-spacer></v-spacer>
    ...
    <v-row>
        <v-col xs="12" sm="12" md="12" class="">
            <v-card elevation="15" class="px-6 py-5 mb-6" style="border-radius: 40px;">
                <v-card-title>
                    <v-btn dark class="mr-5 " elevation="15" color="primary" @click="
                addItem();
                overlay = !overlay;
              ">
                    ...
                    <v-spacer></v-spacer>
                    <v-text-field class="mb-3 mt-3 " v-model="search" append-icon="mdi-magnify" label="ค้นหาข้อมูล" single-line dense hide-details></v-text-field>
                </v-card-title>
                <!-- card main -->
                <v-data-iterator :headers="headers" :items="bank" :search="search" :items-per-page="15" :footer-props="{
              'items-per-page-options': [15, 20, 30, 40, 50, -1],
              'items-per-page-text': 'ข้อมูลหน้าต่อไป'
            }" :sort-by="['datetime']" :sort-desc="[true, false]" class="mb-n5 ">
                    <template v-slot:default="props">
                        <v-row>
                            <v-col v-for="item in props.items" :key="item.name" cols="12" sm="6" md="4" lg="2">
                                <v-card elevation="15" class="rounded-xl " width="400">
                                    <v-img height="200px" contain :aspect-ratio="16 / 9" :src="
                        `${$nuxt.context.env.config.IMG_URL}${item.img_cover}`
                      ">
                                        <v-app-bar flat color="rgba(0, 0, 0, 0)">
                                            <v-toolbar-title class="primary--text pl-0  ml-1" style="height:57px  ">
                                            </v-toolbar-title>
                                        </v-app-bar>
                                    </v-img>
                                    <v-card-actions>
                                        <v-row align="center" justify="space-around" class="mb-1 mt-1">
                                            <v-btn text depressed color="primary accent-4" @click="
                            Detail(item);
                            overlay = !overlay;
                          ">
                                            ...
                                            <v-btn text color="primary accent-4" @click="
                            editItem(item);
                            overlay = !overlay;
                          ">
                                                <span class="primary--text">
