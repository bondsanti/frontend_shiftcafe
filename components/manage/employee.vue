  ...
  <v-data-table
      ...
      >
        <template v-slot:top color="indig">
          <v-toolbar class="mb-2" color="#2A3B4D" dark flat>
        ...
        <template v-slot:item.actions="{ item }">
          <v-btn small class="mr-2" color="info" @click="editItem(item)">
            แก้ไข
          </v-btn>
        ...
        <template v-slot:item.actions.p="{ item }">
          <v-btn depressed color="warning" class="mr-2" @click="editItem(item)">
            เปลี่ยนรหัสผ่าน
          </v-btn>
        </template>
...
export default {
  ...
  methods: {
    initialize() {
      this.loading = true
      this.$axios.$get('/api/employee').then((desserts) => {
